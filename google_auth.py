...
def _get_google_client_config():
    """Get Google OAuth client configuration from environment variables or local_config."""
    # Try environment variables first
    client_id = os.environ.get('GOOGLE_CLIENT_ID')
    client_secret = os.environ.get('GOOGLE_CLIENT_SECRET')
    
    # Try local_config.py if environment variables not set
    if not client_id or not client_secret:
        try:
            from local_config import GOOGLE_CLIENT_ID as LOCAL_CLIENT_ID, GOOGLE_CLIENT_SECRET as LOCAL_CLIENT_SECRET
            client_id = client_id or LOCAL_CLIENT_ID
            client_secret = client_secret or LOCAL_CLIENT_SECRET
        except ImportError:
            pass
    
    # Fallback to placeholder values if not configured
    # In production, always require environment variable or local_config
    if not client_id:
        client_id = 'your-google-client-id-here.apps.googleusercontent.com'
        try:
            current_app.logger.warning("Google Client ID not configured. Set GOOGLE_CLIENT_ID environment variable or add to local_config.py.")
        except RuntimeError:
            print("WARNING: Google Client ID not configured. Set GOOGLE_CLIENT_ID environment variable or add to local_config.py.")
    
    if not client_secret or client_secret.startswith('your') or client_secret.strip() == '':
        client_secret = 'your-google-client-secret-here'
        try:
            current_app.logger.warning("Google Client Secret not configured. Set GOOGLE_CLIENT_SECRET environment variable or add to local_config.py.")
        except RuntimeError:
            print("WARNING: Google Client Secret not configured. Set GOOGLE_CLIENT_SECRET environment variable or add to local_config.py.")
    
    if client_id.startswith('your') or client_secret.startswith('your'):
        error_msg = "Google OAuth credentials are missing or using placeholders. Please set GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET in environment variables or local_config.py."
        try:
            current_app.logger.error(error_msg)
        except RuntimeError:
            # Outside request context, use print
            print(f"ERROR: {error_msg}")
        # Don't raise error - allow app to start but OAuth won't work
